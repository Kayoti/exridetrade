      ...
      <div class="flex flex-col md:flex-row gap-6">
        ...
        <dl class="flex-1 max-w-md text-gray-900 divide-y divide-gray-200 dark:text-white dark:divide-gray-700">
          <div class="flex flex-col pb-3">
            <dt class="mb-1 text-gray-500 md:text-lg dark:text-gray-400">FirstName</dt>
            <dd class="text-lg font-semibold">{{ store.$state.form.firstname || "-" }}</dd>
          </div>
          <div class="flex flex-col pb-3">
            <dt class="mb-1 text-gray-500 md:text-lg dark:text-gray-400">LastName</dt>
            <dd class="text-lg font-semibold">{{ store.$state.form.lastname || "-" }}</dd>
          </div>
          <div class="flex flex-col pt-3">
            <dt class="mb-1 text-gray-500 md:text-lg dark:text-gray-400">Phone number</dt>
            <dd class="text-lg font-semibold">{{ store.$state.form.phone ? `+${store.$state.form.phone}` : "-" }}</dd>
          </div>
          <div class="flex flex-col pb-3">
            <dt class="mb-1 text-gray-500 md:text-lg dark:text-gray-400">Email address</dt>
            <dd class="text-lg font-semibold">{{ store.$state.form.email || "-" }}</dd>
          </div>
          <div class="flex flex-col py-3 border-b border-gray-300 dark:border-gray-600">
            ...
            <dd class="text-lg font-semibold">{{ store.$state.form.location || "-" }}</dd>
          </div>
          <UButton :loading="isSignOutLoading"
        ...
        <ULandingCTA v-bind="cta" class="flex-1 bg-gray-100/50 dark:bg-gray-800/50 p-6" />
      </div>
    </ULandingSection>
  </div>
</template>
