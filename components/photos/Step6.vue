    ...
    <div class=" w-full ">
      ...
      <div class=" w-full flex justify-center items-center h-screen">
        ...
        <div class=" max-h-[90%] p-8 md:p-12 overflow-y-auto rounded-xl shadow-xl md:max-w-3xl mt-3">
            ...
            <h2 class="text-2xl font-bold text-center w-full pb-2">5 of 5</h2> 
            <p class="pt-1 text-center text-sm font-semibold pb-2 px-4">All photos below are required to continue forward.</p>
            <p class="pt-1 text-center text-sm font-semibold pb-2 px-4">Dash with Car on</p>
...
watch(
  () => props.finalImage, async () => {
    ...
    const seatPic = document.getElementById("car-dash");
    seatPic.setAttribute("src", props.finalImage);

    formData.append('car_dash', props.finalImage);
    formData.append('leadid', localStorage.getItem('leadid'));

    store.$state.form.images.car_dash = seatPic.src;
    ...
    if (response.data.status === 'success') {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const newDrivefile = {
            "car_dash": response.data.resp["car_dash"]
          };
